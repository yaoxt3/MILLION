...
from rlpyt.agents.base import AgentInputs, AlternatingRecurrentAgentMixin
...
class RecurrentSacAgent(RecurrentAgentMixin, SacAgent):
    """Base agent for recurrent DQN (to add recurrent mixin)."""

    def __init__(self,
                 ModelCls=FakeRecurrentPiModel,
                 QModelCls=FakeRecurrentQModel,
                 **kwargs):
        """
        ...
        if self.q_model_kwargs is None:
...
class AlternatingRecurrentSacAgent(AlternatingRecurrentAgentMixin, RecurrentSacAgent):
    pass
