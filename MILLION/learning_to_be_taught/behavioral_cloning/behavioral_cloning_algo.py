...
OptInfo = namedtuple("OptInfo", ["loss", "gradNorm", "tdAbsErr", "priority", 'learning_rate'])
...
class BehavioralCloning(RlAlgorithm):
    ...
    def initialize_replay_buffer(self, examples, batch_spec, async_=False):
        ...
        # self.data_loader = iter(DataLoader(TorchDataset(self.replay_buffer), num_workers=0))
        return self.replay_buffer
    ...
    def optimize_agent(self, itr, samples=None, sampler_itr=None):
        ...
        # if self.optimizer_itr % 100 == 0 or self.data_loader is None:
        #     self.data_loader = iter(DataLoader(TorchDataset(self.replay_buffer), num_workers=0))
        ...
        for _ in range(self.updates_per_optimize):
            ...
            # print('loss took: ' + str(time.time() - s))
            s = time.time()
            loss.backward()
            ...
            self.optimizer.step()
            # print('optim took: ' + str(time.time() - s))
            if self.prioritized_replay:
        ...
        # print('full optimizer steps too:' + str(time.time() - start))
        return opt_info
    ...
    def loss(self, samples):
        ...
        if wT > 0:  # Do warmup.
            with torch.no_grad():
                _, _, init_rnn_state = self.agent.pi(warmup_inputs, init_rnn_state)
        ...
        action, _, _ = self.agent.pi(agent_inputs, init_rnn_state)  # [T,B,A]
        delta = torch.sum((action - samples.all_observation.demonstration_actions[self.warmup_T:-1]) ** 2, dim=-1)
        losses = delta
        abs_delta = abs(delta)
        # NOTE: by default, with R2D1, use squared-error loss, delta_clip=None.
        if self.delta_clip is not None:  # Huber loss.
